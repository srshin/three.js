...
export class MapDraw {
    ...
    initGeo(path, color, depth, group){
        ...
        mesh.rotation.z += THREE.Math.degToRad(this.mapInfo.payload.northReference);
    ...
    initOutline(path, color, depth, group) { 
        ...
        lines.rotation.z += THREE.Math.degToRad(this.mapInfo.payload.northReference);
    ...
    initPoiIcon(zCords, defaultFloor) {
        for (let arr of defaultFloor.pois) {
            ...
            imageLoader.load( arr.iconUrl ,  ( image )=> {
                    ...
                    var sqLength = iconSize; 
                    var x = parseInt(this.resizeX(arr.position.x));
                    var y = parseInt(this.resizeY(arr.position.y));
                    console.log('x,y', x,y);
                    ...
                    var depth = 0; 
                    if (zCord == undefined ) {
                        depth = zCords.get("section")+1;
                        console.log(arr.iconUrl + " is not linked with object");
                    } else {
                        depth = zCord+1;
                    }
                    cube.rotation.x = Math.PI;
                    cube.rotation.z += THREE.Math.degToRad(this.mapInfo.payload.northReference);
                    cube.translateZ(  -depth  );
                    cube.translateX( -this.center.x );
                    cube.translateY( -this.center.y );
